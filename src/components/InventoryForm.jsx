...
export default function InventoryForm({ onAdd, editItem, onSave, onCancel, collections = [] }) {
  ...
  const base = 'w-full border border-stone-200 dark:border-stone-600 rounded-xl py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-400 focus:border-transparent transition-shadow dark:bg-stone-700 dark:text-stone-100';
  ...
  return (
    <form onSubmit={handleSubmit} className={isEdit ? '' : 'bg-white dark:bg-stone-800 rounded-2xl shadow-sm border border-stone-100 dark:border-stone-700 p-6'}>
      {!isEdit && <h2 className="text-lg font-semibold text-stone-800 dark:text-stone-100 mb-5">Nuevo artículo</h2>}
